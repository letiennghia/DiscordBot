...
async def on_message(message):
    ...
    for i in username_list:
        ...
        if (wsh.cell(cell.row, 14+datetime.datetime.today().weekday()).value is None):
            wsh.update_cell(
                cell.row, 14+datetime.datetime.today().weekday(), 1)
        else:
            wsh.update_cell(cell.row, 14+datetime.datetime.today().weekday(), float(
                wsh.cell(cell.row, 14+datetime.datetime.today().weekday()).value)+1)
